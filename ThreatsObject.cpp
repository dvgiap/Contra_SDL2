...
void ThreatsObject::ImpMoveType(SDL_Renderer* screen, Map& map_data)
{
	...
	else
	{
		if (on_ground_ == true)
		{
			...
			// copy từ hàm check to map sang do lười quá
			int x1 = 0;
			int x2 = 0;
			int y1 = 0;
			int y2 = 0;
		...
		else
		{
			if (input_type_.left_ == 1)
			{
				LoadImg("images/threat_left.png", screen);
			}
		}
	}
}
...
void ThreatsObject::InitBullet(BulletObject* p_bullet, SDL_Renderer* screen)
{
	if (p_bullet != NULL)
	{
		...
		if (ret)
		{
			...
			p_bullet->SetRect(rect_.x + 5, rect_.y - 700); // set vi tri vien dan dau tien
...
void ThreatsObject::MakeBullet(SDL_Renderer* screen, const int& x_limit)
{
